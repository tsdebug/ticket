...
// Added 'onSuccess' prop to close modal after login
...
export function SignupForm({ className, onSuccess, ...props }: SignupFormProps) {
  ...
  const [isLogin, setIsLogin] = useState(false); // Default to Signup to match your design
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      // 1. Save fake session
      localStorage.setItem("user_token", "demo-token-123");
      localStorage.setItem("user_email", "user@example.com");

      setIsLoading(false);

      // 2. Close modal & Refresh to update Navbar
        ...
        <CardContent className="grid p-0 md:grid-cols-2">
          ...
          <form className="p-6 md:p-8" onSubmit={handleSubmit}>
            <FieldGroup>
              ...
              <Field>
                <Field className={isLogin ? "grid gap-4" : "grid grid-cols-2 gap-4"}>
                  <Field>
                    <div className="flex justify-between items-center">
                        <FieldLabel htmlFor="password">Password</FieldLabel>
                        {isLogin && <span className="text-xs text-purple-600 cursor-pointer">Forgot?</span>}
                    </div>
                    <Input id="password" type="password" required />
                  </Field>
                  
                  {/* Hide Confirm Password if Logging in */}
                  {!isLogin && (
                    <Field>
          ...
          {/* Right side image - kept exactly as is */}
